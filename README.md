# Church Management Second

Quick to Setup Church Management Software
...
Create a database named `ch_manage2`.
...
```bash
celery -A ch_manage2 worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A ch_manage2 worker -l INFO -B --pool=solo
```
